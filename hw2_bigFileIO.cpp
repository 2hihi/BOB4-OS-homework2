...
{
	_ASSERTE(create_very_big_file(L"D:\\test.txt", 1024*4));	// 4GB 파일 생성

	StopWatch sw;
	sw.Start();

	// file copy read write
	_ASSERTE(file_copy_using_read_write(L"D:\\test.txt", L"D:\\test2.txt"));
	sw.Stop();
	...
	FIOHelper.FIOpenForRead(L"D:\\test.txt");
